...
AFRAME.registerComponent("screenshot-fixed", {
  ...
  init: function () {
    ...
    function setup() {
      ...
      self.cubeMapSize = gl.getParameter(
        gl.MAX_CUBE_MAP_TEXTURE_SIZE
      );
      self.material = new THREE.RawShaderMaterial({
        uniforms: { map: { type: "t", value: null } },
        vertexShader: SSVERTEX_SHADER,
  ...
  onKeyDown: function (evt) {
    var shortcutPressed =
      evt.keyCode === 70 && evt.ctrlKey && evt.altKey;
    if (!this.data || !shortcutPressed) {
      return;
    }
    var projection = evt.shiftKey
      ? "equirectangular"
      : "perspective";
  ...
  setCapture: function (projection) {
    ...
    if (projection === "perspective") {
      ...
      el.camera.getWorldQuaternion(
        cubeCamera.quaternion
      );
  ...
  renderCapture: function (camera, size, projection) {
    ...
    if (projection === "perspective") {
      pixels = this.flipPixelsVertically(
        pixels,
        size.width,
        size.height
      );
