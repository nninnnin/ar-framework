...
export async function GET(request: Request) {
  ...
  if (projectId) {
    const projectItem = await projectService.getProject({ projectId });
    return Response.json(formatProjectItem(projectItem));
  }

  const groupName = getSearchParam(request, "groupName");
  if (!groupName) return new Response("groupName is required", { status: 400 });

  const projects = await projectService.getProjects({ groupName });
...
export async function PUT(request: Request) {
  ...
  return await projectService.updateProject(requestBody);
}