    ...
    return this.glbModels
      .map((model) => {
        ...
        if (projectType === "위치기반 AR") {
          ...
          return `<a-entity ${scale} ${position} ${rotation} gps-projected-entity-place="${coordinates}"><a-gltf-model data-model-name="${modelName}" src="#${modelUid}" animation-mixer frustum-culled></a-gltf-model></a-entity>`;
        }
