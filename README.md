...
저는 크리에이티브 스튜디오 [**레벨나인(Rebel9)**](https://rebel9.co.kr)에서 여러가지 인터랙티브 프로젝트들을 개발해왔습니다.  
그 중 Web AR 기술을 기반으로 3D 모델을 다루는 AR 컨텐츠들을 30개 이상 만들었는데요,
...
### 1.1. 지역 현장의 홍보수단: AR 컨텐츠를 담은 모바일 웹 애플리케이션

레벨나인은 여러 지역단체나 기관의 홍보와 관련된 의뢰를 받아 크리에이티브한 비주얼이나 인터랙션으로 사람들의 눈을 모으는 작업을 하는 회사였습니다. AR 컨텐츠는 현장성을 가지면서도 3D 모델을 통해 새로운 경험을 제시할 수 있는 좋은 수단이 되었고, 광주광역시의 대동을 시작으로 제주도 용담동, 성북구의 정릉, 여의도 선유도공원 등을 배경으로 한 프로젝트들을 제작하게 되었습니다.

각 프로젝트는 주로 하나의 모바일 웹 애플리케이션을 통해 지역을 돌아다니며 5~6개의 AR 컨텐츠를 체험할 수 있도록 기획 및 제작되었습니다. 예를 들어 제주도 용담동의 지역개발을 목표로 개발된 프로젝트 <용담플레이>는 7마리의 용들을 찾는다는 컨셉트로, 모바일 웹 애플리케이션을 통해 지도로 여러 위치를 탐색하여 각 장소에 걸맞게 설계된 AR 컨텐츠를 확인하는 방식으로 만들어졌습니다.
