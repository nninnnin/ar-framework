...
저는 크리에이티브 스튜디오 [**레벨나인(Rebel9)**](https://rebel9.co.kr)에서 근무하면서 [AR 컨텐츠를 담은 모바일 웹 애플리케이션](https://dglee.dev?tag=ar)들을 개발했어요. 모바일 웹이라는 접근성 높은 인터페이스를 통해 오프라인 공간에 3D 모델 증강을 통한 새로운 시각적 경험을 제공하여 주로 지역의 특성을 제고시킬 수 있는 홍보 수단으로서 제작되었습니다.

이 AR 컨텐츠들은 정적 HTML 페이지로 제작되어 배포되었고, 별도로 제작된 리액트 앱과 iframe을 이용해 통합되는 일종의 마이크로 프론트엔드 설계를 취했습니다. 문제는 매번 반복적으로 보일러플레이트를 작성해야 했고, 따라서 템플릿의 버저닝이 통합되지 못했으며 따라서 관리와 통합의 비용이 높았습니다. 별개로 3D 모델을 수정하는 작업도 매번 개발자의 손을 거쳐야했어요.

**AR Framework**는 이러한 문제들을 해결하기 위해 자발적으로 제작된 프로덕트입니다. 1년 동안 약 30여개의 반복적인 AR 컨텐츠 개발을 경험하며 쌓은 AR 컨텐츠 제작의 노하우를 하나의 템플릿에 모으고, UI 인터페이스를 통해 개발자 없이도 노코드 환경에서 AR 컨텐츠를 효율적으로 생산하고 편집하며, 완성된 컨텐츠와 외부 개발 환경의 연동을 간편하게 만들기 위한 인터페이스와 도구들의 묶음으로 만들어진 <b>웹 AR 컨텐츠 제작용 프레임워크</b> 입니다.
